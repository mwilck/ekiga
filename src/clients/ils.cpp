...
void GMILSClient::ILSOperation (Operation operation)
{
  ...
  if (operation == ILS_REGISTER) {
    ...
    xml_filename = g_build_filename (DATA_DIR, PACKAGE_NAME,
...
xmlEntityPtr xdap_getentity (void *ctx, const xmlChar * name)
{
  ...
  if ((GnomeMeeting::Process ()->GetManager ()->GetCallingState () != GMManager::Standby)
  ...
  pip = GnomeMeeting::Process ()->GetManager ()->GetCurrentAddress ("h323");
