...
static void
gnomemeeting_aw_modify_addressbook (GtkWidget *addressbook_window, 
				    GmAddressbook *addb, 
				    GmAddressbook *addc);
...
static void gnomemeeting_aw_update_menu_sensitivity (GtkWidget *,
						     ...
						     gboolean);
static void
gm_addressbook_edit_addressbook_dialog_run (GtkWidget *,
					    GmAddressbook *,
					    GtkWidget *);
static void 
	   ...
	   gpointer data)
{
  ...
  contact = get_selected_contact (addressbook_window);
  abook = get_selected_addressbook (addressbook_window);
  
  if (contact)
    gm_addressbook_delete_contact_dialog_run (addressbook_window, 
					      abook, 
					      contact, 
					      addressbook_window);
  else
    gm_addressbook_delete_dialog_run (addressbook_window,
				      abook,
				      addressbook_window);
		    ...
		    gpointer data)
{
  ...
  gm_addressbook_edit_addressbook_dialog_run (addressbook_window,
					      NULL,
					      addressbook_window);
}
	       ...
	       gpointer data)
{
  ...
  if (contact)
    gm_addressbook_edit_contact_dialog_run (addressbook_window,
					    abook, 
					    contact, 
					    addressbook_window);
  else
...
static void
gm_addressbook_edit_addressbook_dialog_run (GtkWidget *addressbook_window,
					    GmAddressbook *addb,
					    GtkWidget *parent_window)
{
  ...
  case GTK_RESPONSE_ACCEPT:
	...
	prefix = g_strdup ("ils");
      ...
      addc->uid = g_strdup_printf ("%s://%s:%s/%s??%s", 
				   prefix, hostname, port, base, scope);
    }
    
    if (addb) {
      
      gnomemeeting_addressbook_modify (addb, addc);
      gnomemeeting_aw_modify_addressbook (parent_window, addb, addc);
    }
    else {
      
      gnomemeeting_addressbook_add (addc);
      gnomemeeting_aw_add_addressbook (parent_window, addc);
    }
                 ...
                 gpointer data)
{
  ...
  gm_addressbook_edit_contact_dialog_run (addressbook,
                                          abook, 
                                          contact, 
                                          addressbook);
...
public:
  ...
  void Main ()
    { 
      PWaitAndSignal m(quit_mutex);
      gdk_threads_enter ();
      addressbook = get_selected_addressbook (addressbook_window);
...
static void
addressbook_search_cb (GtkWidget *w,
                       gpointer data)
{
  GtkWidget *addressbook_window = NULL;
			 ...
			 gpointer data)
{
  ...
  aw = gnomemeeting_aw_get_aw (GTK_WIDGET (data));
  ...
  if (gtk_tree_selection_get_selected (selection, &model, &iter)) {
	...
	gtk_notebook_get_nth_page (GTK_NOTEBOOK (aw->aw_notebook), page_num);
      ...
      if (page)
	awp = gnomemeeting_aw_get_awp (GTK_WIDGET (page));
...
static void
gnomemeeting_aw_add_addressbook (GtkWidget *addressbook_window,
                                 GmAddressbook *addressbook)
{
  GmAddressbookWindow *aw = NULL;
  GmAddressbookWindowPage *awp = NULL;
				    ...
				    GmAddressbook *addc)
{
  ...
  g_return_if_fail (addressbook_window && addb && addc && aw);
  ...
  for (int i = 0 ; i < 2 ; i++) {
    ...
    do {
      ...
      if (test && addb->uid && !strcmp (test, addb->uid)) {

	gtk_tree_store_set (GTK_TREE_STORE (model), &iter,
			    COLUMN_UID, addc->uid,
			    COLUMN_NAME, addc->name,
			    COLUMN_NOTEBOOK_PAGE, p,
			    -1);
	g_free (test);
	break;
...
gm_addressbook_window_new ()
{
  ...
  g_signal_connect (G_OBJECT (find_button), "clicked",
                    G_CALLBACK (addressbook_search_cb),
                    window);
 ...
 */ 
  ...
  while (l) {
   ...
   gnomemeeting_aw_add_addressbook (window, GM_ADDRESSBOOK (l->data));
...
void
gm_addressbook_delete_contact_dialog_run (GtkWidget *addressbook_window,
                                          GmAddressbook *addressbook,
                                          GmContact *contact,
                                          GtkWidget *parent_window)
{
...
void
gm_addressbook_edit_contact_dialog_run (GtkWidget *addressbook_window,
                                        GmAddressbook *addressbook,
                                        GmContact *contact,
                                        GtkWidget *parent_window)
{
